...
async function getMails(email, inbox) {
  ...
  try {
    const response = await fetch(
      `https://mail-box-client-1bc37-default-rtdb.firebaseio.com/mailbox/%20%20%20%20%20%20users/${emailID}/${inbox}.json`
    );
...
async function deleteMail(email, id, inbox) {
  ...
  try {
    const response = await fetch(
      `https://mail-box-client-1bc37-default-rtdb.firebaseio.com/mailbox/%20%20%20%20%20%20users/${emailID}/${inbox}/${id}.json`,
      {
        method: "DELETE",
